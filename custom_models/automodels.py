...
class AugmentModel(nn.Module):
    def __init__(self, N, magn, apply, mode, grad, device):
        ...
        self.device = device
        self.mode = mode # mode
        self.K = 9 # number of affine magnitude params
        self.J = 9 # number of color magnitude params
        K = self.K
        J = self.J
    ...
    def forward(self, idx, x):
        ...
        if self.apply:
            ...
            if mode == 0: #upscale
                ...
                x_warped: torch.tensor = kornia.geometry.transform.warp_perspective(x, M, dsize=(4*H,4*W), padding_mode='border')
                x_warped = kornia.geometry.resize(x_warped, (H,W))
            else:
                x_warped: torch.tensor = kornia.geometry.transform.warp_perspective(x, M, dsize=(H,W), padding_mode='border')
            ## color augmentations
            #if mode == 1:
